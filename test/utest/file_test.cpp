...
ASL_UTEST_CASE(file, file_test) {
	...
	ASL_UTEST_ASSERT_EQ(file.GetPos(), (size_t)nBufferSize);
	file.Close();

	ASL_UTEST_ASSERT_TRUE(file.Open(szFilename, File::OF_In));
	ASL_UTEST_ASSERT_EQ(file.GetPos(), (size_t)0);
	ASL_UTEST_ASSERT_EQ(file.GetFileSize(), (size_t)nBufferSize);
	...
	ASL_UTEST_ASSERT_EQ(file.Write(acBuffer1, nBufferSize), nBufferSize);
	ASL_UTEST_ASSERT_EQ(file.GetFileSize(), (size_t)(2 * nBufferSize));
	ASL_UTEST_ASSERT_EQ(file.GetPos(), (size_t)(2 * nBufferSize));
	file.Close();
